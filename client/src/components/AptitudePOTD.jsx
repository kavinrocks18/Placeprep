...
import { Brain, Clock, ArrowRight, Lightbulb, Tag } from 'lucide-react';
...
const AptitudePOTD = () => {
    ...
    useEffect(() => {
        const fetchPOTD = async () => {
            try {
                const { data } = await api.get('/aptitude/potd');
                setProblem(data);
            } catch (error) {
                console.error('Error fetching Aptitude POTD:', error);
            } finally {
                setLoading(false);
            }
        };
    ...
    if (!problem) {
        return (
            <div className="bg-card border border-dashed border-border/30 rounded-xl p-6 text-center">
                ...
                <p className="text-sm text-muted-foreground">No aptitude problem available today.</p>
            </div>
        );
    }
        ...
        >
            ...
            <div className="p-6">
                ...
                <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-xs text-amber-400/80 font-medium">{q.category}</span>
                </div>
                ...
                <div className="flex items-center justify-between pt-2 border-t border-border/20">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-muted-foreground">{q.timeLimit || 120}s time limit</span>
                    </div>
                    <Link
                        to={`/aptitude/${q.slug}`}
                        className="inline-flex items-center gap-1.5 px-4 py-2 bg-amber-500 text-black rounded-lg text-sm font-bold hover:bg-amber-400 transition-all shadow-md shadow-amber-500/15"
                    >
