...
const ProblemDetail = () => {
    ...
    const handleRunSuccess = async () => {
        try {
            if (question) {
                await api.put(`/users/solve/${question._id}`);
            }
        } catch (error) {
    ...
    if (!question) {
        return (
            <div className="text-center py-20">
                ...
                <Link to="/practice" className="text-primary hover:underline text-sm mt-2 inline-block">← Back to Practice</Link>
            </div>
        );
    }
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-8rem)] animate-fade-in-up">
            <Link to="/practice" className="inline-flex items-center text-sm text-muted-foreground hover:text-foreground mb-4 w-fit transition-colors">
