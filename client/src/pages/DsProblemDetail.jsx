...
import { ArrowLeft, CheckCircle, Database, Tag } from 'lucide-react';
import CodeEditor from '../components/CodeEditor';

const DsProblemDetail = () => {
    ...
    useEffect(() => {
        const fetchQuestion = async () => {
            try {
                const { data } = await api.get(`/ds/${slug}`);
                setQuestion(data);
            } catch (error) {
                console.error('Error fetching DS question:', error);
            } finally {
                setLoading(false);
            }
        };
    ...
    if (!question) {
        return (
            <div className="text-center py-20">
                ...
                <Link to="/data-structures" className="text-primary hover:underline text-sm mt-2 inline-block">← Back to Data Structures</Link>
            </div>
        );
    }
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-8rem)] animate-fade-in-up">
            <div className="flex items-center justify-between mb-4">
                <Link to="/data-structures" className="inline-flex items-center text-sm text-muted-foreground hover:text-foreground w-fit transition-colors">
                    <ArrowLeft className="h-4 w-4 mr-1" /> Back to Data Structures
                </Link>
...
export default DsProblemDetail;
